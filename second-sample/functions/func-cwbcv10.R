runcwbcv10=function(Y,indice,lag, type = "cv10"){
  ...
  if(sum(dum)==1){
    X2=cbind(X,dum)
    colnames(X2)=c(nomes = paste0("V", 1:504), "dum")
  }else{
    X2=X
    colnames(X2)=c(nomes = paste0("V", 1:504))
  }
  ...
  if (type == "aic") { 
    ...
    m_opt <- mstop(aic_obj)
    cat("M_opt selecionado:", m_opt, "de um máximo de 500\n") 
  ...
  if (type == "cv10") {
    outlier_idx <- which.min(y)
    cv10f <- cv(model.weights(best_fit), type = "kfold", B = 10)
    cv10f[outlier_idx, ] <- 1
    cvm <- cvrisk(best_fit, folds = cv10f, papply = lapply)
    m_opt <- mstop(cvm)
    model <- best_fit[m_opt]
    cat("Optimal number of boosting iterations:",m_opt,"\n" )
  }
  ...
  coef_opt <- rep(0, ncol(X2))
  
  if(sum(dum)==1){
    names(coef_opt) <- c(paste0("V", 1:504), "dum")
  }else{
    names(coef_opt) <- c(paste0("V", 1:504))
  }
...
cwbcv10.rolling.window=function(Y,nprev,indice=1,lag=1){
  
  
  save.pred=matrix(NA,nprev,1)
  for(i in nprev:1){
    Y.window=Y[(1+nprev-i):(nrow(Y)-i),]
    bo=runcwbcv10(Y.window,indice,lag)
    aux=bo$model$coef
    if(length(aux)<37){
      aux=c(aux,0)
    }
