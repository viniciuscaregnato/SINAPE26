...
factorboosting.rolling.window=function(Y,nprev,indice=1,lag=1){
  
  save.coef=matrix(NA,nprev,37)
  save.pred=matrix(NA,nprev,1)
  for(i in nprev:1){
    Y.window=Y[(1+nprev-i):(nrow(Y)-i),]
    bo=runfactorboosting(Y.window,indice,lag)
    aux=bo$model$coef
    if(length(aux)<37){
      aux=c(aux,0)
    }
    save.coef[(1+nprev-i),]=aux
